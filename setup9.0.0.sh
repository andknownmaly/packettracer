...
function install_packettracer() {
    ...
    if [ ! -f "/tmp/CiscoPacketTracer_900_Ubuntu_64bit.deb" ]; then
        ...
        if command -v xterm &> /dev/null; then
            xterm -e "wget https://github.com/andknownmaly/packettracer/releases/download/9.0.0/CiscoPacketTracer_900_Ubuntu_64bit.deb"
        else
    ...
    sudo dpkg-deb -x ./CiscoPacketTracer_900_Ubuntu_64bit.deb / 2>&1 | grep -v "tar:"
    ...
    if [ -f "/opt/pt/packettracer.AppImage" ]; then
        ...
        # Old installation method for version 8.x
        ...
        # Check if lib directory exists to set proper LD_LIBRARY_PATH
...
function uninstall_packettracer() {
    ...
    rm -f "$HOME/.local/share/applications/CiscoPacketTracer-9.0.0.desktop"
    rm -f "$HOME/.local/share/applications/CiscoPacketTracerPtsa-9.0.0.desktop"
    rm -f "$HOME/.local/share/icons/hicolor/256x256/apps/packettracer.png"
    rm -rf "$HOME/.config/Packet Tracer"
...
while true; do
    ...
    if command -v figlet &> /dev/null; then
        ...
        echo -e "${CYAN}${BOLD}"
        echo "╔════════════════════════════════════════════════════╗"
        echo "║         CISCO PACKET TRACER INSTALLER 9.0.0       ║"
        echo "╚════════════════════════════════════════════════════╝"
        echo -e "${NC}"
    fi
    ...
    echo ""
    echo -e "${YELLOW}═══════════════════════════════════════════════════${NC}"
    echo -e "${BOLD}1.${NC} Install Packet Tracer 9.0.0"
